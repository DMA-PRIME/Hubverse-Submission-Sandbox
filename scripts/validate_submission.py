#!/usr/bin/env python3
"""Dependency-free teaching validator for the DMA-PRIME submission sandbox."""
...
def main(args: list[str]) -> int:
    ...
    if args and len(metadata_paths) != 1:
        results.errors.append(
            f"A practice pull request must contain exactly one metadata file; found {len(metadata_paths)}."
        )
    if args and len(output_paths) != 1:
        results.errors.append(
            f"A practice pull request must contain exactly one output file; found {len(output_paths)}."
        )
    ...
    output_ids = {path.parent.name for path in output_paths}
    for model_id in sorted(metadata_ids - output_ids):
        metadata_path = next(path for path in metadata_paths if path.stem == model_id)
        results.error(relative(metadata_path), f"missing model-output/{model_id}/<round_id>-{model_id}.<extension>")
    if args and "student-demo" in metadata_ids | output_ids:
        results.errors.append(
            "student-demo is the repository's reserved example; use the anonymous practice ID generated by the guide."
        )
